...
namespace Diligent
{
	...
	{
	public:
		ImGuiDiligentRenderer(IRenderDevice* pDevice, TEXTURE_FORMAT BackBufferFmt, TEXTURE_FORMAT DepthBufferFmt, Uint32 InitialVertexBufferSize, Uint32 InitialIndexBufferSize, float scale);
		~ImGuiDiligentRenderer();
		void NewFrame(Uint32 RenderSurfaceWidth, Uint32 RenderSurfaceHeight, SURFACE_TRANSFORM SurfacePreTransform, float scale);
		void EndFrame();
		void RenderDrawData(IDeviceContext* pCtx, ImDrawData* pDrawData);
		void InvalidateDeviceObjects();
		void CreateDeviceObjects(float scale);
		void CreateFontsTexture(float scale);

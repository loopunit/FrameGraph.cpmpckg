...
namespace mu
{
	namespace details
	{
		...
		struct gfx_window_impl : public gfx_window
		{
			...
			struct
			{
				std::atomic<std::uint32_t> m_viewport_begin_frame_task_counter;
				std::atomic<std::uint32_t> m_viewport_end_imgui_task_counter;
				std::atomic<std::uint32_t> m_viewport_end_frame_task_counter;
			} m_task_state;

			virtual [[nodiscard]] auto begin_frame(tf::Subflow& subflow) noexcept -> mu::leaf::result<void>
			{
				auto self = this;
				MU_LEAF_CHECK(self->m_application_state->make_current());

				tf::Task main_window = subflow
										   .emplace(
											   [&self]()
											   {
												   if (auto func_error = [&self]() -> mu::leaf::result<void>
													   {
														   MU_LEAF_CHECK(self->m_application_state->make_current());

														   MU_LEAF_CHECK(self->update_dpi());

														   MU_LEAF_CHECK(self->init_resources());

														   if (self->m_diligent_window) [[likely]]
														   {
															   MU_LEAF_CHECK(self->m_diligent_window->create_resources(self->m_display_size[0], self->m_display_size[1]));
														   }

														   self->m_task_state.m_viewport_begin_frame_task_counter = 1;
					...
					for (int n = 1; n < platform_io.Viewports.Size; n++)
					{
						tf::Task child_window =
							subflow
										...
										if (auto func_error = [&self]() -> mu::leaf::result<void>
											{
												...
												auto			 n			 = self->m_task_state.m_viewport_begin_frame_task_counter++;
												ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
												ImGuiViewport*	 viewport	 = platform_io.Viewports[n];
												...
												{
													auto wnd = static_cast<gfx_child_window*>(viewport->PlatformUserData);
													MU_LEAF_CHECK(wnd->begin_frame(self->m_diligent_window->m_globals, self->m_imgui_renderer->m_shared_resources));
												}
								...
								.name("per_window");
						child_window.succeed(main_window);
					}
			...
			virtual [[nodiscard]] auto begin_imgui(tf::Subflow& subflow) noexcept -> mu::leaf::result<void>
			{
				auto self = this;
							...
							if (auto func_error = [&self]() -> mu::leaf::result<void>
								{
									try
									{
										...
										MU_LEAF_CHECK(self->new_frame());

										ImGuiViewport* main_viewport = ImGui::GetMainViewport();

										MU_LEAF_CHECK(self->update_dpi());

										MU_LEAF_CHECK(self->m_imgui_shared_resources->create_device_objects(self->m_dpi_scale, false));
										ImGuiIO& io		= ImGui::GetIO();
										io.Fonts->TexID = (ImTextureID)self->m_imgui_shared_resources->m_font_srv;
			...
			virtual [[nodiscard]] auto end_imgui(tf::Subflow& subflow) noexcept -> mu::leaf::result<void>
			{
				auto self = this;
				MU_LEAF_CHECK(self->m_application_state->make_current());
												   ...
												   if (auto func_error = [&self]() -> mu::leaf::result<void>
													   {
														   ...
														   ImGui::EndFrame();
														   ImGui::UpdatePlatformWindows();
														   self->m_task_state.m_viewport_end_imgui_task_counter = 0;
															...
															if (auto func_error = [&self]() -> mu::leaf::result<void>
																{
																	...
																	auto n = self->m_task_state.m_viewport_end_imgui_task_counter++;

																	if (n == 0)
																	{
																		MU_LEAF_CHECK(self->render(ImGui::GetDrawData()));
																	}
																	else
																	{
																		ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
																		...
																		{
																			auto wnd = static_cast<gfx_child_window*>(viewport->PlatformUserData);
																			MU_LEAF_CHECK(wnd->render(self->m_renderer_globals->m_immediate_context, viewport->DrawData));
																		}
																	}
			...
			virtual [[nodiscard]] auto end_frame(tf::Subflow& subflow) noexcept -> mu::leaf::result<void>
			{
				auto self = this;
				MU_LEAF_CHECK(self->m_application_state->make_current());
												   ...
												   if (auto func_error = [&self]() -> mu::leaf::result<void>
													   {
														   ...
														   self->m_task_state.m_viewport_end_frame_task_counter = 0;
															...
															if (auto func_error = [&self]() -> mu::leaf::result<void>
																{
																	...
																	auto n = self->m_task_state.m_viewport_end_frame_task_counter++;

																	if (n == 0)
																	{
																		if (self->m_diligent_window && self->m_diligent_window->m_swap_chain)
																		{
																			MU_LEAF_CHECK(self->m_diligent_window->present());
																		}
																	}
																	else
																	{
